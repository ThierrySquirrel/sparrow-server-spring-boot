 ...
 **/
package io.github.thierrysquirrel.sparrow.server.hummingbird.core.container;
...
public class ModularMethodContainer {
    private static final Map<Modular, Map<Event, ModularMethod>> METHOD_CONTAINER = new ConcurrentHashMap<>();

    private ModularMethodContainer() {
    }

    public static void putMethodDomain(Modular modular, Event event, ModularMethod modularMethod) {
        METHOD_CONTAINER.computeIfAbsent(modular, key -> new ConcurrentHashMap<>())
